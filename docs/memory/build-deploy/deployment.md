...
## Inbound help refresh (scheduled pull)

Beyond developer pushes, there is one automated inbound path to `main`: a scheduled job that refreshes `help/<slug>.json` at the repo root (the [help-collection](../conventions/help-collection.md) consume/pull side; forward contract in [`docs/specs/help-dump-contract.md`](../../specs/help-dump-contract.md)). As of change `oa63` (2026-06-03) this is a **pull**, not the prior push model. The flow:
