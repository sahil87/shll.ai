...
| Date | Change |
|------|--------|
| 2026-06-08 | Consumer-fix reconciliation (change `ng8c`): a `..`-**escape** now rewrites to a reserved **`/tools/<slug>/__unresolved__/…` marker** (a visibly-dead link) instead of the old silent clamp to a real page — and the escape math is single-sourced as `resolvePath`, shared by both rewriters and `escapesDocsSite` (so rewrite and lint agree). The closure lint's image scan now covers **`<source srcset>`** (each candidate URL), not just markdown + `<img src>`. **`RESERVED_SLUGS` shrank to `{overview, readme, commands}`** (was 5-item): `install`/`workflows` are no longer reserved — they belong to the tool repo via `docs/site/`, and the hallucinated site-authored `idea`/`fab-kit` install/workflows static pages + their sidebar/nav refs were **removed** (contract PRs #41/#42 + §Producer conformance directive). Old `resolveSegments` clamp helper removed. See [readme-extraction](./readme-extraction.md) for the sibling README-slice link lint added in the same change. |
