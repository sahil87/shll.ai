...
The CLI adds a **second report-only lint**: a **reserved-slug `::warning::`** when a docs/site page's top mount segment ∈ `{overview, readme, commands}` (`RESERVED_SLUGS`). Such a page would mount at `/tools/<slug>/<reserved>` and — because the dynamic route is higher-priority than Starlight's catch-all — **silently shadow** the site-owned page (the hand-authored `overview` + the generated `readme`/`commands`). Same posture: warn (visible in the run log), commit anyway, fix in the tool repo (spec §9.2 makes it a producer rule). **`install` and `workflows` are NOT reserved** (change `ng8c`, contract PRs #41/#42): they belong to the tool repo via `docs/site/install.md` / `docs/site/workflows.md` (rendered at `/tools/<slug>/install|workflows`) — the site's prior hand-authored `install`/`workflows` stubs for `idea` and `fab-kit` were site-authored prose with no canonical source (the hand-copied drift *Tool-Page Depth* forbids) and were **removed**, freeing those slugs for the tool's own docs/site content.
