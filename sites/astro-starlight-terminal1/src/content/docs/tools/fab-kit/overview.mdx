...
`fab-kit` is the engine of the loop: it turns an **[idea](/idea/)** backlog item into a planned change (via `/fab-new`), runs that change inside a disposable **[wt](/wt/)** worktree, and you watch every agent session from the **[run-kit](/run-kit/)** dashboard.
